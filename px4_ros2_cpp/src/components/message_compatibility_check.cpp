...
namespace {
std::string messageFieldsStrForMessageHash(rclcpp::Node& node, const std::string& topic_type,
                                           const std::string& msgs_dir)
{
  ...
  static const std::set<std::string> kBasicTypes{{"bool"},    {"byte"},   {"char"},   {"float32"},
                                                 {"float64"}, {"int8"},   {"uint8"},  {"int16"},
  ...
  for (std::sregex_iterator iter(text.begin(), text.end(), kMsgFieldTypeRegex);
    ...
    if (type_iter == kBasicTypes.end()) {  // Nested type
      if (type.find('/') == std::string::npos) {
        ...
        RCLCPP_ERROR(node.get_logger(), "Field %s contains namespace %s", filename.c_str(),
                     type.c_str());
      }
    }
  }
...
inline constexpr uint32_t hash32Fnv1aConst(const char* const str,
                                           const uint32_t value = kVal32Const) noexcept
{
  return (str[0] == '\0')
             ? value
             : hash32Fnv1aConst(&str[1], (value ^ static_cast<uint32_t>(str[0])) * kPrime32Const);
}

uint32_t messageHash(rclcpp::Node& node, const std::string& topic_type, const std::string& msgs_dir)
{
      ...
      if (wait_ret.kind() == rclcpp::WaitResultKind::Ready) {
        ...
        if (message_format_response_sub->take(response, info)) {
          if (response.protocol_version ==
            ...
            if (strcmp(reinterpret_cast<const char*>(response.topic_name.data()),
            ...
            RCLCPP_ERROR(node.get_logger(), "Protocol version mismatch: got %i, expected %i",
                               ...
                               const std::string& topic_namespace_prefix)
{
  ...
  if (px4_ros2::isRmwZenoh()) {
    ...
    RCLCPP_WARN(node.get_logger(),
                "ROS2 Humble and earlier do not support type hash, thus no type checking will "
                "occur when using Zenoh. It's recommended to use Zenoh with ROS2 Jazzy or later");
#else
    ...
    for (const auto& message_to_check : messages_to_check) {
      ...
      strncpy(reinterpret_cast<char*>(request.topic_name.data()),
      ...
      switch (requestMessageFormat(node, request, message_format_response_sub,
                                   message_format_request_pub, response, first_message)) {
        ...
        case RequestMessageFormatReturn::GotReply:
          if (response.success) {
            ...
            RCLCPP_FATAL(node.get_logger(), "MessageFormatResponse::success == false for %s",
