...
MapProjection::MapProjection(Context& context) : _node(context.node())
{
                              ...
                              const Eigen::Vector2d& global_position_next)
{
  ...
  const float x = static_cast<float>(cos(lat_now_rad) * sin(lat_next_rad) -
                                     sin(lat_now_rad) * cos_lat_next * cos(d_lon));
                                       ...
                                       const Eigen::Vector2d& global_position_next)
{
  ...
  vector_ne.x() =
      static_cast<float>(kRadiusOfEarth * (cos(lat_now_rad) * sin(lat_next_rad) -
                                                 ...
                                                 float heading, float dist)
{
  ...
  const double lat_target_rad =
      asin(sin(lat_now_rad) * cos(radius_ratio) +
           cos(lat_now_rad) * sin(radius_ratio) * cos(static_cast<double>(heading)));
  const double lon_target_rad =
      lon_now_rad + atan2(sin(static_cast<double>(heading)) * sin(radius_ratio) * cos(lat_now_rad),
                                          ...
                                          const Eigen::Vector2f& vector_ne)
{
  ...
  global_position_res.y() = radToDeg(lon_now_rad + static_cast<double>(vector_ne.y()) /
