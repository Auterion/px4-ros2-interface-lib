...
void OnResume::run(const std::shared_ptr<ActionHandler>& handler, const ActionArguments& arguments,
...
void OnResume::resumeFromUnexpectedLanding(const std::shared_ptr<ActionHandler>& handler,
                                           const std::function<void()>& on_completed,
                                           int current_mission_index)
{
  ...
  // Find takeoff action and run it (if there are multiple, take the one closest to the current
  // index)
  ...
  if (takeoff_action_item) {
    handler->runAction(takeoff_action_item->name, takeoff_action_item->arguments,
...
void OnResume::navigateToPreviousWaypoint(const std::shared_ptr<ActionHandler>& handler,
                                          const std::function<void()>& on_completed,
                                          int current_mission_index)
{
      ...
      std::visit(util::Overloaded{[&](const Waypoint& waypoint) {
                   switch (waypoint.frame) {
                     case MissionFrame::Global:
                       const auto mission =
                           std::make_shared<Mission>(std::vector<MissionItem>{waypoint});
...
void OnFailure::run(const std::shared_ptr<ActionHandler>& handler, const ActionArguments& arguments,
                    const std::function<void()>& on_completed)
{
  // Try RTL first, then fall back to Descend
  handler->runMode(ModeBase::kModeIDRtl, on_completed, [handler, on_completed] {
    handler->runMode(ModeBase::kModeIDDescend, on_completed, [handler, on_completed] {
      // If this one fails, do nothing
    });
  });
}

void ChangeSettings::run(const std::shared_ptr<ActionHandler>& handler,
