...
void MissionExecutor::runModeTakeoff(
  ...
  const std::function<void()> & on_completed, const std::function<void()> & on_failure)
{
  ...
  RCLCPP_DEBUG(
    _node.get_logger(), "Running takeoff mode with altitude: %f, heading: %f",
    altitude, heading);
  _mission_item_state = MissionItemState::Other;
    ...
    util::Overloaded{
      [&](const NavigationItem & navigation_item)
      {
        ...
        runTrajectory(
          ...
          [end_index = end_index, this, action_handler = _action_handler](int index)
          {
