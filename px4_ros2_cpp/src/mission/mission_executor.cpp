...
class AsyncFunctionCalls {
 public:
  explicit AsyncFunctionCalls(rclcpp::Node& node) : _node(node)
  {
...
MissionExecutor::MissionExecutor(const std::string& mode_name, const Configuration& configuration,
                                 rclcpp::Node& node, const std::string& topic_namespace_prefix)
    : _persistence_filename(configuration.persistence_filename),
      _node(node),
      _reporting(std::make_unique<AsyncFunctionCalls>(node))
{
  _mode = std::make_unique<MissionMode>(node, mode_name, topic_namespace_prefix, *this);
  _mode_executor = std::make_unique<MissionModeExecutor>(node, ModeExecutorBase::Settings{}, *_mode,
                                                         topic_namespace_prefix, *this);
  ...
  _readyness_timer = _node.create_wall_timer(1s, [this] { checkReadynessAndReport(); });
}

MissionExecutor::~MissionExecutor()  // NOLINT destructor needs to be in .cpp file for forward
                                     // declaration
{
}
  ...
  for (const auto& mission_item : mission.items()) {
    std::visit(util::Overloaded{
                   [&](const NavigationItem& navigation_item) {
                     std::visit(
                         util::Overloaded{[&](const Waypoint& waypoint) {
                           if (!_trajectory_executor->navigationItemTypeSupported(
                   ...
                   [&](const ActionItem& action_item) {
                     if (_actions.find(action_item.name) == _actions.end()) {
                       _mission_errors.push_back("Mission contains unknown action '" +
                                                 action_item.name + "'");
                     }
                   },
               },
...
std::unique_ptr<ActionStateKeeper> MissionExecutor::addContinousAction(const ActionState& state)
{
  ...
  RCLCPP_DEBUG(_node.get_logger(), "Adding continuous action id %i, action name '%s'", id,
               state.name.c_str());
...
{
  if (!_is_active) {
    // If the state is removed as a result of the deactivate() call, we want to keep it so it can be
    // restored later on (if not stored persistently)
    return;
  }
...
{
  // Create a copy of the actions, and reset them, as they are expected to be added again after
  // running them
  ...
  for (const auto& action : actions) {
    ...
    if (iter == _actions.end()) {
      RCLCPP_WARN(_node.get_logger(), "Stored action name '%s' not found",
  ...
  // We expect all restored actions to have created another continuous action, so the action counts
  // should be equal
  if (!actions.empty()) {
    RCLCPP_DEBUG(_node.get_logger(), "Restored %lu actions (new stored count: %lu)", actions.size(),
                 _state.continuous_actions.size());
  }
}
...
void MissionExecutor::checkReadynessAndReport()
{
  ...
  if (_on_readyness_update) {
    ...
    _reporting->callAsync(
        [this, all_errors] { _on_readyness_update(all_errors.empty(), all_errors); });
  }
}
...
void MissionExecutor::updateSetpoint()
{
  if (_mission_item_state != MissionItemState::Trajectory) {
    // After triggering a mode switch we might still get an update setpoint request until the mode
    // actually switches
    ...
    if (_node.get_clock()->now() > *_trajectory_update_warn + 1s) {
      ...
      RCLCPP_ERROR_THROTTLE(_node.get_logger(), *_node.get_clock(), 1000,
                            "Trying to update setpoint, but no trajectory set");
    }
    return;
  }
...
void MissionExecutor::onActivate()
{
  ...
  if (!isReady()) {
    RCLCPP_ERROR(_node.get_logger(),
                 "Activated without valid mission, or actions not ready, aborting");
  ...
  if (_on_activated) {
    const auto previous_handler = _action_handler;
    _on_activated();
    if (!previous_handler->isValid()) {
      // If the handler got invalidated (e.g. due to an abort()), do not run the actions
      return;
    }
  }
  ...
  if (_actions.find("onResume") == _actions.end() || start_from_beginning) {
    ...
    runAction("onResume", ActionArguments(arguments),
...
void MissionExecutor::runMode(ModeBase::ModeID mode_id, const std::function<void()>& on_completed,
      ...
      mode_id, [this, on_completed, on_failure, mode_id](const Result& result) {
        if (result == Result::Success) {
          ...
          if (on_failure) {
            ...
            RCLCPP_ERROR(_node.get_logger(), "Mode %i failed (%s), aborting", mode_id,
                                     ...
                                     const std::function<void()>& on_failure)
{
  ...
  RCLCPP_DEBUG(_node.get_logger(), "Running takeoff mode with altitude: %f, heading: %f", altitude,
               heading);
  _mission_item_state = MissionItemState::Other;
      ...
      [this, on_completed, on_failure](const Result& result) {
        if (result == Result::Success) {
          ...
          if (on_failure) {
            ...
            RCLCPP_ERROR(_node.get_logger(), "Takeoff mode failed (%s), aborting",
                         resultToString(result));
            abort(AbortReason::ModeFailure);
          }
        }
...
void MissionExecutor::runAction(const std::string& action_name, const ActionArguments& arguments,
                                const std::function<void()>& on_completed)
{
  if (!_is_active) {
...
void MissionExecutor::runTrajectory(const std::shared_ptr<Mission>& trajectory, int start_index,
                                    int end_index, const std::function<void(int)>& on_index_reached,
                                    bool stop_at_last_item)
{
  ...
  RCLCPP_DEBUG(_node.get_logger(), "Running trajectory (start: %i, end: %i, stop: %i)", start_index,
  ...
  const TrajectoryExecutorInterface::TrajectoryConfig config{
      trajectory,        _trajectory_options, start_index, end_index,
...
void MissionExecutor::runCurrentMissionItem(bool resuming)
{
  ...
  if (!_mission->indexValid(*_state.current_index)) {
    ...
    // Otherwise, we assume the previous mode triggered a landing (e.g. via RTL). This is not
    // necessarily true, but we cannot directly look at the landing state here, as it might not be
    // set yet.
    if (_active_mode != _mode->id()) {
  ...
  std::visit(util::Overloaded{[&](const NavigationItem& navigation_item) {
                                const auto [end_index, should_stop_at_index] =
                                    getNextTrajectorySegment(*_state.current_index);
                                runTrajectory(
                                    _mission, *_state.current_index, end_index,
                                    [end_index = end_index, this,
                                     action_handler = _action_handler](int index) {
                                      // As the callback might still be called after the mission is
                                      // deactivated or aborted, we check if the previous handler is
                                      // still valid
                                      if (action_handler->isValid()) {
                              ...
                              [&](const ActionItem& action_item) {
                                ...
                                runAction(action_item.name, ActionArguments(arguments),
...
void MissionExecutor::abort(AbortReason reason)
{
  ...
  RCLCPP_DEBUG(_node.get_logger(), "Aborting mission (reason: %s, recursion level: %i)",
  ...
  if (_abort_recursion_level > 10) {
    ...
    // Reaching the limit should generally be fixed in the onFailure action, rather than trying to
  ...
  if (_actions.find("onFailure") == _actions.end()) {
    ...
    runAction("onFailure", ActionArguments(arguments), [this] {
      RCLCPP_DEBUG(_node.get_logger(), "onFailure action completed");
      // We expect the failure action to handle everything, so do not try to do anything in addition
      // here. We could reset the mission state here (resetMissionState()), but it might not be
      // desired in all cases, for example when the flight controller deactivates the executor and
      // at the same time the mission executor wants to switch modes (race condition).
    });
  }
  --_abort_recursion_level;
}
...
void ActionHandler::runTrajectory(const std::shared_ptr<Mission>& trajectory,
                                  const std::function<void()>& on_completed, bool stop_at_last_item)
{
  if (!_valid) {
    RCLCPP_WARN(_mission_executor._node.get_logger(), "ActionHandler is not valid anymore");
    return;
  }
  RCLCPP_DEBUG(_mission_executor._node.get_logger(), "Running custom trajectory (%lu items)",
    ...
    if (const auto* navigation_item = std::get_if<NavigationItem>(&item)) {
      std::visit(util::Overloaded{[&](const Waypoint& waypoint) {
                   ...
                   if (!_mission_executor._trajectory_executor->frameSupported(waypoint.frame)) {
                     RCLCPP_ERROR(_mission_executor._node.get_logger(),
  ...
  _mission_executor.runTrajectory(
      trajectory, 0, end_index,
      [on_completed, end_index](int index) {
        if (index == end_index) {
          on_completed();
        }
      },
...
void ActionHandler::setCurrentMissionIndex(int index)
{
  if (!_mission_executor._mission->indexValid(index)) {
    RCLCPP_ERROR(_mission_executor._node.get_logger(), "setCurrentMissionIndex: invalid index %i",
                 index);
    return;
  }
