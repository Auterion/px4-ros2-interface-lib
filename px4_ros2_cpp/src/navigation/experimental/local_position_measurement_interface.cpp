...
LocalPositionMeasurementInterface::LocalPositionMeasurementInterface(
    rclcpp::Node& node, const PoseFrame pose_frame, const VelocityFrame velocity_frame,
    ...
    const LocalPositionMeasurement& local_position_measurement) const
{
  ...
  if (!isFrameValid(local_position_measurement)) {
    throw NavigationInterfaceInvalidArgument("Measurement has an unknown reference frame.");
  }
  ...
  aux_local_position.timestamp_sample =
      local_position_measurement.timestamp_sample.nanoseconds() * 1e-3;
  ...
  const Vector2f position_xy_variance =
      local_position_measurement.position_xy_variance.value_or(Vector2f{NAN, NAN});
  ...
  // Attitude
  const Quaternionf attitude_quaternion =
      local_position_measurement.attitude_quaternion.value_or(Quaternionf{NAN, NAN, NAN, NAN});
  ...
  const Vector3f attitude_variance =
      local_position_measurement.attitude_variance.value_or(Vector3f{NAN, NAN, NAN});
  ...
  const Vector2f velocity_xy_variance =
      local_position_measurement.velocity_xy_variance.value_or(Vector2f{NAN, NAN});
...
{
  return measurement.position_xy.has_value() || measurement.position_z.has_value() ||
         measurement.velocity_xy.has_value() || measurement.velocity_z.has_value() ||
    ...
    const LocalPositionMeasurement& measurement) const
{
  ...
  if (measurement.position_z.has_value() &&
    ...
    RCLCPP_ERROR_ONCE(_node.get_logger(),
                      "Measurement value position_z has an invalid variance value.");
    return false;
  }
  ...
  if (measurement.velocity_z.has_value() &&
    ...
    RCLCPP_ERROR_ONCE(_node.get_logger(),
                      "Measurement value velocity_z has an invalid variance value.");
    return false;
  }
...
bool LocalPositionMeasurementInterface::isFrameValid(
    const LocalPositionMeasurement& measurement) const
{
  if ((measurement.position_xy.has_value() || measurement.position_z.has_value()) &&
      _pose_frame == AuxLocalPosition::POSE_FRAME_UNKNOWN) {
    RCLCPP_ERROR_ONCE(_node.get_logger(), "Position measurement has unknown pose frame.");
    return false;
  }
  ...
  if ((measurement.velocity_xy.has_value() || measurement.velocity_z.has_value()) &&
    ...
    RCLCPP_ERROR_ONCE(_node.get_logger(), "Velocity measurement has unknown velocity frame.");
    return false;
  }
...
bool LocalPositionMeasurementInterface::isValueNotNAN(
    const LocalPositionMeasurement& measurement) const
{
  ...
  if (measurement.position_xy_variance.has_value() &&
    ...
    RCLCPP_ERROR_ONCE(_node.get_logger(),
                      "Measurement value position_xy_variance is defined but contains a NAN.");
    return false;
  }
  if (measurement.position_z.has_value() && std::isnan(measurement.position_z.value())) {
    RCLCPP_ERROR_ONCE(_node.get_logger(),
                      "Measurement value position_z is defined but contains a NAN.");
    return false;
  }
  if (measurement.position_z_variance.has_value() &&
      std::isnan(measurement.position_z_variance.value())) {
    RCLCPP_ERROR_ONCE(_node.get_logger(),
                      "Measurement value position_z_variance is defined but contains a NAN.");
    return false;
  }
  ...
  if (measurement.velocity_xy_variance.has_value() &&
    ...
    RCLCPP_ERROR_ONCE(_node.get_logger(),
                      "Measurement value velocity_xy_variance is defined but contains a NAN.");
    return false;
  }
  if (measurement.velocity_z.has_value() && std::isnan(measurement.velocity_z.value())) {
    RCLCPP_ERROR_ONCE(_node.get_logger(),
                      "Measurement value velocity_z is defined but contains a NAN.");
    return false;
  }
  if (measurement.velocity_z_variance.has_value() &&
      std::isnan(measurement.velocity_z_variance.value())) {
    RCLCPP_ERROR_ONCE(_node.get_logger(),
                      "Measurement value velocity_z_variance is defined but contains a NAN.");
    return false;
  }
  if (measurement.attitude_quaternion.has_value() &&
      measurement.attitude_quaternion.value().coeffs().hasNaN()) {
    RCLCPP_ERROR_ONCE(_node.get_logger(),
                      "Measurement value attitude_quaternion is defined but contains a NAN.");
    return false;
  }
  if (measurement.attitude_variance.has_value() && measurement.attitude_variance.value().hasNaN()) {
    RCLCPP_ERROR_ONCE(_node.get_logger(),
                      "Measurement value attitude_variance is defined but contains a NAN.");
    return false;
  }
  return true;
}
