...
class OdometryLocalPosition : public Subscription<px4_msgs::msg::VehicleLocalPosition> {
 public:
   ...
   * data but doesn't necessarily require local position to be available (for instance if the mode
   * reads altitude data from vehicle_local_position). Reading XY position without a correct source
   * of positional data is not recommended
   */
  explicit OdometryLocalPosition(Context& context, bool local_position_is_optional = false);

  bool positionXYValid() const { return lastValid() && last().xy_valid; }

  bool positionZValid() const { return lastValid() && last().z_valid; }
  ...
  bool velocityXYValid() const { return lastValid() && last().v_xy_valid; }

  bool velocityZValid() const { return lastValid() && last().v_z_valid; }
