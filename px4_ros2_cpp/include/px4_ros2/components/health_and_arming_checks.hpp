...
class HealthAndArmingCheckReporter {
 public:
  ...
  void armingCheckFailureExt(uint32_t event_id, events::Log log_level, const char* message,
                             Args... args)
  {
 ...
 private:
  template <typename... Args>
  bool addEvent(uint32_t event_id, const events::LogLevels& log_levels, const char* message,
                Args... args);
                                            ...
                                            const char* message, Args... args)
{
  ...
  e.id = event_id;
  static_assert(events::util::sizeofArguments(args...) <= sizeof(e.arguments),
                "Too many arguments");
...
inline void HealthAndArmingCheckReporter::setHealth(uint8_t health_component_index, bool is_present,
...
class HealthAndArmingChecks {
 public:
   ...
   * @param name registration name. Should uniquely identify the component with length < 25
   * characters
