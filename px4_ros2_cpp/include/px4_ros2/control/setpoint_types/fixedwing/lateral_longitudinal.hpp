...
class FwLateralLongitudinalSetpointType : public SetpointBase {
 public:
   ...
   * can be set for full flexibility. See
   * https://docs.px4.io/main/en/msg_docs/FixedWingLateralSetpoint.html and
   * https://docs.px4.io/main/en/msg_docs/FixedWingLongitudinalSetpoint.html for more information on
   * this setpoint type.
   *
   * @param config a FwControlConfiguration object where pitch, throttle, lateral acceleration
   * configuration can be set. See
   * https://docs.px4.io/main/en/msg_docs/LateralControlConfiguration.html and
   * https://docs.px4.io/main/en/msg_docs/LongitudinalControlConfiguration.html for more information
  ...
  /**
   * @brief Update the setpoint with simplest set of inputs: Altitude and Course. Lateral
   * acceleration setpoints will be used as feedforward when course is provided. To directly control
   * lateral acceleration, set course_sp to NAN.
   *
   * @warning Any previously set configurations will be maintained when this method is called.
   * If no configurations have been previously set, PX4 will set default configurations.
   */
  void updateWithAltitude(float altitude_amsl_sp, float course_sp,
                          std::optional<float> equivalent_airspeed_sp = {},
                          std::optional<float> lateral_acceleration_sp = {});

  /**
   * @brief Update the setpoint with simplest set of inputs: Height rate and Course. Lateral
   * acceleration setpoints will be used as feedforward when course is provided. To directly control
   * lateral acceleration, set course_sp to NAN.
   *
   * @warning Any previously set configurations will be maintained when this method is called.
   * If no configurations have been previously set, PX4 will set default configurations.
   */
  void updateWithHeightRate(float height_rate_sp, float course_sp,
                            std::optional<float> equivalent_airspeed_sp = {},
                            std::optional<float> lateral_acceleration_sp = {});
...
struct FwControlConfiguration {
  ...
  /**
   * @brief Configure the speed weight for TECS (Total Energy Control System). For more information
   * on TECS, see
   * https://docs.px4.io/main/en/flight_stack/controller_diagrams.html#total-energy-control-system-tecs
   *
   * @param speed_weight_sp speed weight setpoint in the range of [0.0, 2.0]. Speedweight is 1.0 by
   * default. Set speedweight to 0.0 for altitude priority, 2.0 for airspeed priority.
   */
