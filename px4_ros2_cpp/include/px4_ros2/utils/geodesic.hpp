 ...
 *
 * This class handles ROS2 subscription and initializing the map projection reference point from
 * PX4. The mathematical implementation is contained in `px4_ros2::MapProjectionImpl`.
 *
...
/**
 * @brief Compute the global position on the line vector defined by two positions (start -> end) at
 * a certain distance from global position 'start'.
 *
 * @param global_position_line_start line start lat [deg], lon [deg]
 * @param global_position_line_end line end lat [deg], lon [deg]
 * @param dist_from_start distance [m] of target global position from position 'start' towards
 * position 'end' (can be negative)
 *
 * @ingroup geodesic
 */
...
/**
 * @brief Compute the global position from adding a local frame vector to the current global
 * position.
 *
