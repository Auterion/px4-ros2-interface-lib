...
class ActionArguments {
 public:
  ...
  const nlohmann::json& json() const { return *_data; }
...
enum class MissionFrame {
  Global,  ///< WGS84 latitude/longitude and altitude is mean sea level (MSL), corresponds to
           ///< MAV_FRAME_GLOBAL
};
...
class Mission {
 public:
  ...
  explicit Mission(std::vector<MissionItem> mission_items, MissionDefaults mission_defaults = {})
...
class MissionFileMonitor {
 public:
  MissionFileMonitor(std::shared_ptr<rclcpp::Node> node, std::filesystem::path filename,
