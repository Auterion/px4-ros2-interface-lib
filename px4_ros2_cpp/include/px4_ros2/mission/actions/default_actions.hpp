...
class Rtl : public ActionInterface {
 public:
  ...
  void run(const std::shared_ptr<ActionHandler>& handler, const ActionArguments& arguments,
...
class Hold : public ActionInterface {
 public:
  explicit Hold(ModeBase& mode) : _node(mode.node()) {}
           ...
           const std::function<void()>& on_completed) override
  {
    ...
    _timer =
        _node.create_wall_timer(std::chrono::duration<float>(duration_s), [this, on_completed] {
          _timer.reset();
          on_completed();
        });
  }
...
class OnResume : public ActionInterface {
 ...
 private:
  void resumeFromUnexpectedLanding(const std::shared_ptr<ActionHandler>& handler,
                                   const std::function<void()>& on_completed,
                                   int current_mission_index);
  void navigateToPreviousWaypoint(const std::shared_ptr<ActionHandler>& handler,
                                  const std::function<void()>& on_completed,
                                  int current_mission_index);
