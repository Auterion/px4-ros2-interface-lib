...
/**
 * @brief Mission execution state machine
 * @ingroup mission
 */
class MissionExecutor {
 public:
  struct Configuration {
    ...
    {
      trajectory_executor_factory = [args = std::tie(std::forward<Args>(args)...)](
                                        ModeBase& mode) mutable {
        return std::apply(
            [&mode](auto&&... args) {
  ...
  MissionExecutor(const std::string& mode_name, const Configuration& configuration,
  ...
  /**
   * Get notified when the current mission item changes. This is the index into the current mission
   * items that is currently being executed (i.e. if it is a waypoint, the current target waypoint)
   */
  void onProgressUpdate(const std::function<void(int)>& callback)
  {
    _on_progress_update = callback;
  }
   ...
   *
   * @param activity_info An optional string containing additional information about the current
   * action.
   */
  ...
  /**
   * Abort a currently running mission. This will trigger the onFailure action (which can be
   * customized). The abort reason will be set to 'other'.
   */
  void abort();
 ...
 protected:
  class MissionMode : public ModeBase {
   public:
    MissionMode(rclcpp::Node& node, const Settings& settings,
  ...
  class MissionModeExecutor : public ModeExecutorBase {
   public:
    MissionModeExecutor(rclcpp::Node& node, const Settings& settings, ModeBase& owned_mode,
    ...
    Result sendCommandSync(uint32_t command, float param1, float param2, float param3, float param4,
                           float param5, float param6, float param7) override
    {
      if (command_handler) {
        return command_handler(command, param1) ? Result::Success : Result::Rejected;
      }
      return ModeExecutorBase::sendCommandSync(command, param1, param2, param3, param4, param5,
                                               param6, param7);
    }
 ...
 private:
  ...
  void runMode(ModeBase::ModeID mode_id, const std::function<void()>& on_completed,
   ...
   * @param on_failure optional callback to execute when the takeoff fails (if not set, abort is
   * called instead)
   */
  void runModeTakeoff(float altitude, float heading, const std::function<void()>& on_completed,
                      const std::function<void()>& on_failure = nullptr);
  void runAction(const std::string& action_name, const ActionArguments& arguments,
                 const std::function<void()>& on_completed);
  void runTrajectory(const std::shared_ptr<Mission>& trajectory, int start_index, int end_index,
  ...
  int _abort_recursion_level{
      0};  ///< abort() might be called recursively, this stores the current level
  ...
  std::unique_ptr<AsyncFunctionCalls> _reporting;  // Report asynchronously to avoid potential
                                                   // recursive calls and state changes in between
...
class ActionStateKeeper {
 public:
  ...
  ~ActionStateKeeper() { _mission_executor.removeContinousAction(_id); }
...
/**
 * @brief Handler passed to custom actions to run modes, actions and trajectories
 * @ingroup mission
 */
class ActionHandler {
 public:
  explicit ActionHandler(MissionExecutor& mission_executor) : _mission_executor(mission_executor) {}
  ...
  void runTrajectory(const std::shared_ptr<Mission>& trajectory,
   ...
   *        making it externally accessible. This string provides a concise description of the
   * current action.
   *
   ...
   *
   * This is primarily for the onResume action, in case the mission should be resumed at a different
   * index.
   *
   * When called as part of another action, the index will still be increased, and thus continue at
   * the *next* item.
   */
  void setCurrentMissionIndex(int index);
   ...
   *
   * This is used to restore the state when the mission is interrupted (e.g. by the user or a
   * failsafe) and continued later on. When restoring, the action will be called with the provided
   * arguments.
   *
   * This can be used for example, for a camera trigger command that continuous to run in the
   * background.
   * @param action_name Name of the calling action
   * @param arguments Arguments to the action for restoring the action
   * @return A token that should be stored. It can be deleted when the action stops or gets
   * deactivated
   */
  std::unique_ptr<ActionStateKeeper> storeState(const std::string& action_name,
                                                const ActionArguments& arguments)
  {
    ...
    return _mission_executor.addContinousAction(
        MissionExecutor::ActionState{action_name, arguments});
  }
   ...
   *
   * If the handler is invalid, it cannot be used anymore. It will be set invalid when the mission
   * is aborted, or deactivated. This is useful for actions that use timers to run actions.
   */
