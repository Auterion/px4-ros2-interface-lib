...
class Battery : public Subscription<px4_msgs::msg::BatteryStatus> {
 public:
  explicit Battery(Context& context)
      : Subscription<px4_msgs::msg::BatteryStatus>(
            context, "fmu/out/battery_status" +
                         px4_ros2::getMessageNameVersion<px4_msgs::msg::BatteryStatus>())
  {
  }
   ...
   */
  float voltageV() const { return last().voltage_v; }
   ...
   */
  float currentA() const { return last().current_a; }
   ...
   */
  float dischargedCapacityMah() const { return last().discharged_mah; }
   ...
   */
  float remaningFraction() const { return last().remaining; }
   ...
   */
  uint8_t cellCount() const { return last().cell_count; }

  /**
   * @brief Get the voltages of each cell in the vehicle's battery.
   *
   * @return an Eigen vector containing the cell voltages. If the cell count is unknown, returns an
   * empty vector.
   */
