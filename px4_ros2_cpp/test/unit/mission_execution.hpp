 ...
 public:
  ...
  void updateSetpoint() override
  {
    ...
    if (const auto* navigation_item =
      ...
      std::visit(
          px4_ros2::util::Overloaded{[&](const px4_ros2::Waypoint& waypoint) {
            RCLCPP_INFO(_node.get_logger(), "Waypoint[%i]: %.6f %.6f %.3f, frame: %i", _index,
...
class MissionExecutorTest : public px4_ros2::MissionExecutor {
 public:
  ...
  MissionExecutorTest(const std::string& mode_name, const Configuration& configuration,
                      rclcpp::Node& node, std::shared_ptr<FakeAutopilot> fake_autopilot)
      : MissionExecutor(mode_name, configuration, node, kTopicPrefix),
        _node(node),
        _fake_autopilot(std::move(fake_autopilot))
  {
    setCommandHandler([this](uint32_t command, float param1) {
    ...
    _land_detected->onUpdate(
        [this](const px4_msgs::msg::VehicleLandDetected& landed) { onLanded(landed); });

    // Set a default for changing modes
    mode_change_request = [this](uint8_t nav_state) {
      const bool send_completed =
          nav_state != modeId() && nav_state != px4_ros2::ModeBase::kModeIDLoiter;
  ...
  void setModeAndArm(uint8_t mode_id, bool send_completed = true)
  {
    ...
    if (send_completed) {
      ...
      _mode_continue_timer = _node.create_wall_timer(std::chrono::milliseconds(10),
                                                     [this]() { handleModeContinueState(); });
    } else {
 ...
 private:
  ...
  void onLanded(const px4_msgs::msg::VehicleLandDetected& landed) { handleModeContinueState(); }
  rclcpp::Node& _node;
