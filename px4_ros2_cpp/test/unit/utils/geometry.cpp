...
{
  EXPECT_NEAR(180.f, px4_ros2::radToDeg(px4_ros2::kPi), 1e-3);
  EXPECT_NEAR(-720.f, px4_ros2::radToDeg(-4.f * px4_ros2::kPi), 1e-3);
  EXPECT_NEAR(60.f, px4_ros2::radToDeg(px4_ros2::kPi / 3.f), 1e-3);
...
{
  EXPECT_NEAR(px4_ros2::kPi, px4_ros2::degToRad(180.f), 1e-3);
  EXPECT_NEAR(-4.f * px4_ros2::kPi, px4_ros2::degToRad(-720.f), 1e-3);
  EXPECT_NEAR(px4_ros2::kPi / 3.f, px4_ros2::degToRad(60.f), 1e-3);
}

TEST(Geometry, wrapPi)
{
  EXPECT_NEAR((px4_ros2::kPi / 2), px4_ros2::wrapPi((px4_ros2::kPi / 2)), 1e-3);
  EXPECT_NEAR(-(px4_ros2::kPi / 2), px4_ros2::wrapPi(-(px4_ros2::kPi / 2)), 1e-3);
  EXPECT_NEAR(0.f, px4_ros2::wrapPi(4 * px4_ros2::kPi), 1e-3);
  EXPECT_NEAR(-3.f * px4_ros2::kPi / 4.f, px4_ros2::wrapPi(-11.f * px4_ros2::kPi / 4.f), 1e-3);
  EXPECT_NEAR(3.f * px4_ros2::kPi / 4.f, px4_ros2::wrapPi(11.f * px4_ros2::kPi / 4.f), 1e-3);

  // Absolute value due to floating point precision, 2*k*pi can map to -pi and pi
  EXPECT_NEAR(px4_ros2::kPi, std::fabs(px4_ros2::wrapPi(-11.0 * px4_ros2::kPi)), 1e-3);
}
...
TEST(Geometry, quaternionToEulerRpy)
{
  ...
  // Roll
  q = Eigen::AngleAxisf((px4_ros2::kPi / 2), Eigen::Vector3f::UnitX());
  v_euler = Eigen::Vector3f{(px4_ros2::kPi / 2), 0.f, 0.f};
  vectorsApproxEqualTest(v_euler, px4_ros2::quaternionToEulerRpy(q), "roll pi/2");

  // Pitch
  q = Eigen::AngleAxisf(-px4_ros2::kPi / 4.f, Eigen::Vector3f::UnitY());
  v_euler = Eigen::Vector3f{0.f, -px4_ros2::kPi / 4.f, 0.f};
  vectorsApproxEqualTest(v_euler, px4_ros2::quaternionToEulerRpy(q), "pitch -pi/4");

  // Yaw
  q = Eigen::AngleAxisf(3.f * px4_ros2::kPi / 4.f, Eigen::Vector3f::UnitZ());
  v_euler = Eigen::Vector3f{0.f, 0.f, 3.f * px4_ros2::kPi / 4.f};
  vectorsApproxEqualTest(v_euler, px4_ros2::quaternionToEulerRpy(q), "yaw 3pi/4");

  // Gimbal lock cases
  q = Eigen::AngleAxisf(-(px4_ros2::kPi / 2), Eigen::Vector3f::UnitY());
  v_euler = Eigen::Vector3f{0.f, -(px4_ros2::kPi / 2), 0.f};
  ...
  q = Eigen::AngleAxisf(0.1f, Eigen::Vector3f::UnitX()) *
      Eigen::AngleAxisf(0.1f, Eigen::Vector3f::UnitY()) *
      Eigen::AngleAxisf(px4_ros2::kPi - 0.1f, Eigen::Vector3f::UnitZ());
  quaternionToEulerReconstructionTest(q);
...
TEST(Geometry, eulerRpyToQuaternion)
{
  ...
  v_euler = Eigen::Vector3f{0.1f, 0.1f, px4_ros2::kPi - 0.1f};
  quaternionsApproxEqualTest(q, px4_ros2::eulerRpyToQuaternion(v_euler));
  quaternionsApproxEqualTest(q,
                             px4_ros2::eulerRpyToQuaternion(v_euler.x(), v_euler.y(), v_euler.z()));
...
TEST(Geometry, quaternionToRoll)
{
  ...
  q = Eigen::AngleAxisf(-px4_ros2::kPi / 4.f, Eigen::Vector3f::UnitY()) *
      Eigen::AngleAxisf((px4_ros2::kPi / 2), Eigen::Vector3f::UnitZ());
  roll = -px4_ros2::kPi / 4.f;
  EXPECT_NEAR(roll, px4_ros2::quaternionToRoll(q), 1e-3);
}
...
TEST(Geometry, quaternionToYaw)
{
  ...
  // 90 deg yaw
  q = Eigen::AngleAxisf((px4_ros2::kPi / 2), Eigen::Vector3f::UnitZ());
  yaw = (px4_ros2::kPi / 2);
