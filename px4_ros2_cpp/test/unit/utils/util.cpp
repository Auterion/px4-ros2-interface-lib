...
void quaternionsApproxEqualTest(const Eigen::Quaternionf& q_expected,
                                const Eigen::Quaternionf& q_actual, const std::string& msg,
                                const float precision)
{
  ...
  EXPECT_TRUE(q_expected.isApprox(q_actual, precision) ||
              q_expected_negated.isApprox(q_actual, precision))
      << "test: " << msg << '\n'
      << "  q_actual: " << q_actual.coeffs().transpose() << '\n'
      << "q_expected: " << q_expected.coeffs().transpose() << '\n';
}

void vectorsApproxEqualTest(const Eigen::Vector3d& v_expected, const Eigen::Vector3d& v_actual,
...
void vectorsApproxEqualTest(const Eigen::Vector3f& v_expected, const Eigen::Vector3f& v_actual,
                            const std::string& msg, const float precision)
{
  vectorsApproxEqualTest(v_expected.cast<double>(), v_actual.cast<double>(), msg,
