...
TEST(MissionExecutor, json)
{
  ...
  EXPECT_TRUE(fabsf(mission.defaults().trajectory_options.horizontal_velocity.value() - 12.5f) < 0.00001f);
...
TEST(MissionExecutor, fileMonitor)
{
  ...
  const px4_ros2::MissionFileMonitor monitor(node, kFilename, [&](const std::shared_ptr<px4_ros2::Mission>& m)
  {
    mission = m;
  });
...
TEST_F(MissionExecutionTester, resumeFromLandedWithResumeSupport)
{
  ...
  MissionExecutorTest mission_executor("my mission",
    px4_ros2::MissionExecutor::Configuration()
    .addCustomAction<CustomActionInterruptible>(on_action_run, num_run_calls, supports_resume_from_landed)
    .withTrajectoryExecutor<TrajectoryExecutorTest>(),
    *node, fake_autopilot);
  ASSERT_TRUE(mission_executor.doRegister());
  mission_executor.setMission(mission);
...
public:
  ...
  {
    RCLCPP_INFO(_node.get_logger(), "Stopping camera trigger (from deactivate), was active: %s", _state ? "yes" : "no");
    _state.reset();
  }
private:
...
TEST_F(MissionExecutionTester, resumeWithActions)
{
  ...
  const px4_ros2::Mission mission({px4_ros2::ActionItem("takeoff"),
    px4_ros2::Waypoint({47.34820919, 8.54395214, 503}),
    px4_ros2::ActionItem("changeSettings", px4_ros2::ActionArguments{ {{ "horizontalVelocity", 15.0 }} }),
    px4_ros2::Waypoint({47.34820919, 8.54395214, 504}),
    px4_ros2::ActionItem("customActionCameraTrigger", px4_ros2::ActionArguments{ {{ "action", "start" }} }),
    px4_ros2::ActionItem("changeSettings", px4_ros2::ActionArguments{ {{ "horizontalVelocity", 18.0 }} }),
    px4_ros2::ActionItem("changeSettings", px4_ros2::ActionArguments{ {{ "maxHeadingRate", 55.0 }} }),
    px4_ros2::Waypoint({47.34720919, 8.54385214, 502}),
    px4_ros2::ActionItem("customActionInterruptible", { }),
    px4_ros2::ActionItem("customActionCameraTrigger", px4_ros2::ActionArguments{ {{ "action", "stop" }} }),
    px4_ros2::Waypoint({47.39820919, 8.54595214, 500}),
    px4_ros2::ActionItem("changeSettings", px4_ros2::ActionArguments{ {{ "resetAll", true }} }),
    px4_ros2::Waypoint({47.39820919, 8.54595214, 501}),
    px4_ros2::ActionItem("changeSettings", px4_ros2::ActionArguments{ {{ "verticalVelocity", 7.5 }} }),
  ...
  auto init_mission_executor = [&]
  {
    ...
    auto mission_executor = std::make_shared<MissionExecutorTest>("my mission",
      px4_ros2::MissionExecutor::Configuration().withPersistenceFile(persistence_file)
      .addCustomAction<CustomActionInterruptible>(on_action_run, num_run_calls)
  ...
  // Simulate reboot. The mission should be continued, and actions restored
  {
    ...
    fake_autopilot->setLanded(true);
    mission_executor->landDetected()->onUpdate([&land_detected](const px4_msgs::msg::VehicleLandDetected& vehicle_land_detected) { land_detected = vehicle_land_detected.landed; });
...
TEST_F(MissionExecutionTester, failureDefault)
{
  ...
  MissionExecutorTest mission_executor("my mission",
    px4_ros2::MissionExecutor::Configuration().addCustomAction<CustomActionTriggerFailure>(), *node, fake_autopilot);
  ASSERT_TRUE(mission_executor.doRegister());
  mission_executor.setMission(mission);
...
public:
    ...
    const std::function<void()> & on_completed) override
  {
    ...
    RCLCPP_INFO(_node.get_logger(), "Failure handling: recursion level: %i, reason: %s, current index: %i", recursion_level, reason.c_str(), index);
    handler->runAction("nonExistentAction", {}, []
    {
      EXPECT_TRUE(false) << "on_completed called, but should never be";
    });
...
TEST_F(MissionExecutionTester, failureCustom)
{
  ...
  MissionExecutorTest mission_executor("my mission",
    px4_ros2::MissionExecutor::Configuration().addCustomAction<CustomActionTriggerFailure>().addCustomAction<CustomOnFailureAction>(), *node, fake_autopilot);
  ASSERT_TRUE(mission_executor.doRegister());
  mission_executor.setMission(mission);
...
TEST_F(MissionExecutionTester, readyness)
{
  ...
  // The mission contains some errors
  {
    ...
    const auto expected = std::vector<std::string>{{std::string("Mission contains unknown action 'unknownAction'"), "Action 'customActionReadyness' is not ready yet"}};
  ...
  action_can_run = true;
  {
    ...
    const auto expected = std::vector<std::string>{{std::string("Mission contains unknown action 'unknownAction'")}};
    EXPECT_EQ(last_errors, expected);
  }
