 ...
 *   - Set up: LocalPositionMeasurementInterface instance is configured with PoseFrame::LocalNED and
 ...
 *   - Set up: LocalPositionMeasurementInterface instance is configured with PoseFrame::Unknown and
 * VelocityFrame::LocalNED.
 *   - Test Cases:
 *     1. VelocityFrameUnknown: Sends a measurement with a position, and a measurement with a
 * velocity. Expects success for velocity measurements, exception for position measurements.
 */
 ...
 protected:
  void SetUp() override
  {
    ...
    _local_navigation_interface = std::make_shared<px4_ros2::LocalPositionMeasurementInterface>(
        *_node, px4_ros2::PoseFrame::LocalNED, px4_ros2::VelocityFrame::LocalNED);
  }
};
...
class LocalPositionInterfacePoseTest : public LocalPositionInterfaceTestBase {
 protected:
  void SetUp() override
  {
    LocalPositionInterfaceTestBase::SetUp();
    _local_navigation_interface = std::make_shared<px4_ros2::LocalPositionMeasurementInterface>(
        *_node, px4_ros2::PoseFrame::LocalNED, px4_ros2::VelocityFrame::Unknown);
  }
};

// Configure interface with PoseFrame::Unknown and VelocityFrame::LocalNED
class LocalPositionInterfaceVelocityTest : public LocalPositionInterfaceTestBase {
 protected:
  void SetUp() override
  {
    LocalPositionInterfaceTestBase::SetUp();
    _local_navigation_interface = std::make_shared<px4_ros2::LocalPositionMeasurementInterface>(
        *_node, px4_ros2::PoseFrame::Unknown, px4_ros2::VelocityFrame::LocalNED);
  }
};
...
TEST_F(LocalPositionInterfaceTest, MeasurementEmpty)
{
  ...
  EXPECT_THROW(_local_navigation_interface->update(measurement), NavigationInterfaceInvalidArgument)
...
TEST_F(LocalPositionInterfacePoseTest, PoseFrameUnknown)
{
  ...
  EXPECT_NO_THROW(_local_navigation_interface->update(measurement))
      << "Failed to send position update (position_xy) with known pose frame and unknown velocity "
         "frame.";
  ...
  measurement.position_z_variance = 0.33F;
  EXPECT_NO_THROW(_local_navigation_interface->update(measurement))
      << "Failed to send position update (position_z) with known pose frame and unknown velocity "
         "frame.";
  ...
  measurement.velocity_xy_variance = Eigen::Vector2f{0.3f, 0.4f};
  EXPECT_THROW(_local_navigation_interface->update(measurement), NavigationInterfaceInvalidArgument)
      << "Expected exception for velocity update (velocity_xy) in unknown velocity frame, but none "
         "was thrown.";
  ...
  measurement.velocity_z_variance = 0.33F;
  EXPECT_THROW(_local_navigation_interface->update(measurement), NavigationInterfaceInvalidArgument)
      << "Expected exception for velocity update (velocity_z) in unknown velocity frame, but none "
         "was thrown.";
}
...
TEST_F(LocalPositionInterfaceVelocityTest, VelocityFrameUnknown)
{
  ...
  measurement.position_xy_variance = Eigen::Vector2f{0.2f, 0.1f};
  EXPECT_THROW(_local_navigation_interface->update(measurement), NavigationInterfaceInvalidArgument)
      << "Expected exception for position update (position_xy) in unknown pose frame, but none was "
         "thrown.";
  ...
  measurement.position_z_variance = 0.33F;
  EXPECT_THROW(_local_navigation_interface->update(measurement), NavigationInterfaceInvalidArgument)
      << "Expected exception for position update (position_z) in unknown pose frame, but none was "
         "thrown.";
  ...
  measurement.velocity_xy_variance = Eigen::Vector2f{0.3f, 0.4f};
  EXPECT_NO_THROW(_local_navigation_interface->update(measurement))
      << "Failed to send velocity update (velocity_xy) with known velocity frame and unknown pose "
         "frame.";
  ...
  measurement.velocity_z_variance = 0.33F;
  EXPECT_NO_THROW(_local_navigation_interface->update(measurement))
      << "Failed to send velocity update (velocity_z) with known velocity frame and unknown pose "
         "frame.";
}
