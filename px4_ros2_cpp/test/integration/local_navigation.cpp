 ...
 *   - Test Cases:
 *     1. FuseEvPos: Sends position measurements, expects cs_ev_pos and cs_ev_hgt flags to be set to
 * true.
...
TEST_F(LocalPositionInterfaceTest, fuseEvPos)
{
  ...
  waitForMeasurementUpdate(std::move(measurement),
                           [](const EstimatorStatusFlags::UniquePtr& flags) {
...
TEST_F(LocalPositionInterfaceTest, fuseEvVel)
{
  ...
  waitForMeasurementUpdate(
      ...
      [](const EstimatorStatusFlags::UniquePtr& flags) { return flags->cs_ev_vel; });
}
...
TEST_F(LocalPositionInterfaceTest, fuseEvYaw)
{
  ...
  waitForMeasurementUpdate(
      std::move(measurement),
      [](const EstimatorStatusFlags::UniquePtr& flags) { return flags->cs_ev_yaw; });
}
