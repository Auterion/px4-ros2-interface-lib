...
std::shared_ptr<rclcpp::Node> initNode()
{
  ...
  auto ret = rcutils_logging_set_logger_level(test_node->get_logger().get_name(),
                                              RCUTILS_LOG_SEVERITY_DEBUG);

  if (ret != RCUTILS_RET_OK) {
    RCLCPP_ERROR(test_node->get_logger(), "Error setting severity: %s",
  ...
  _vehicle_status_sub = _node.create_subscription<px4_msgs::msg::VehicleStatus>(
      ...
      [this](px4_msgs::msg::VehicleStatus::UniquePtr msg) {
        ...
        if (_on_mode_set_callback && _waiting_for_nav_state == msg->nav_state) {
          if (++_matching_nav_state_set >
              2) {  // wait a bit longer for the mode to be set, and mode activation triggered
...
void VehicleState::sendCommand(uint32_t command, float param1, float param2, float param3,
