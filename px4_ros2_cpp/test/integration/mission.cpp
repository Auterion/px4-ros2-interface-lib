...
class TestMission {
 public:
  explicit TestMission(rclcpp::Node& node) : _node(node), _vehicle_state(node) {}
...
void TestMission::run()
{
  ...
  _mission = std::make_unique<MissionExecutorTest>(
      kName, px4_ros2::MissionExecutor::Configuration(), _node);
  ...
  const px4_ros2::Mission mission({px4_ros2::ActionItem("takeoff"), waypoints[0], waypoints[1],
                                   waypoints[2], px4_ros2::ActionItem("land")});
  _mission->setMission(mission);
  std::vector<bool> waypoints_reached(waypoints.size(), false);

  const float acceptance_radius = 2.0f;
  global_position.onUpdate([&](const px4_msgs::msg::VehicleGlobalPosition& msg) {
    for (std::size_t i = 0; i < waypoints.size(); i++) {
      if (waypoints_reached[i]) {
        continue;
      }
      if (px4_ros2::distanceToGlobalPosition(global_position.position(), waypoints[i].coordinate) <
          acceptance_radius) {
        ...
        if (_state == State::WaitForArming) {
          if (msg->pre_flight_checks_pass) {
            ...
            _mission->getModeExecutor().arm(
                [](px4_ros2::Result result) { ASSERT_EQ(result, px4_ros2::Result::Success); });
          }
        }
      });
