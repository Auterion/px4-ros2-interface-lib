...
void TestExecution::run()
{
      ...
      [this](const px4_msgs::msg::VehicleStatus::UniquePtr& msg) {
        switch (_state) {
          ...
          case State::WaitUntilCannotArm:
            ...
            // Ensure we got a vehicle status update after we reported a failing check (prevent race
            // conditions)
  ...
  _health_and_arming_checks = std::make_unique<px4_ros2::HealthAndArmingChecks>(
      _node, [this](px4_ros2::HealthAndArmingCheckReporter& reporter) {
        if (_state == State::WaitForFirstCallback) {
          ...
          /* EVENT
           */
          reporter.armingCheckFailureExt(px4_ros2::events::ID("check_unit_test_failure"),
