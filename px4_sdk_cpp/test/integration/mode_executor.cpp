  ...
  void runState(State state, px4_sdk::Result previous_result)
  {
    ...
    switch (state) {
      ...
      case State::WaitUntilDisarmed:
        waitUntilDisarmed([this](px4_sdk::Result result) {runState(State::Completed, result);});
        break;
...
void TestExecutionAutonomous::run()
{
  ...
  _mode_executor->on_state_completed = [](ModeExecutorTest::State, px4_sdk::Result result) {
        ...
        if (msg->pre_flight_checks_pass) {
          ...
          _mode_executor->arm(
            [this](px4_sdk::Result result) {
              ASSERT_EQ(result, px4_sdk::Result::Rejected);
              ...
              _vehicle_state.callbackOnModeSet(
                [this]() {
                  ...
                  _mode_executor->arm(
                    [this](px4_sdk::Result result) {
                      ASSERT_EQ(result, px4_sdk::Result::Success);
  ...
  _mode_executor->on_state_completed =
    [this](ModeExecutorTest::State next_state, px4_sdk::Result result) {
      ...
      if (_state == State::WaitForRTL && next_state == ModeExecutorTest::State::RTL) {
        ...
        EXPECT_TRUE(result == px4_sdk::Result::Deactivated || result == px4_sdk::Result::Rejected);                 // Switching to land deactivated the executor
  ...
  _mode_executor->on_state_completed =
    [this](ModeExecutorTest::State next_state, px4_sdk::Result result) {
      ...
      if (next_state == ModeExecutorTest::State::TakingOff) {
        ...
        EXPECT_EQ(result, px4_sdk::Result::Deactivated);                 // Expect the failsafe to have triggered in MyMode
