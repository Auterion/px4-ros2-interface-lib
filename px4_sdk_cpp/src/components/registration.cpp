...
: _node(node)
{
  ...
  _unregister_ext_component.mode_id = px4_sdk::ModeBase::kModeIDInvalid;
}
...
bool Registration::doRegister(const RegistrationSettings & settings)
{
  ...
  request.px4_sdk_api_version = kLatestPX4SdkApiVersion;
      ...
      if (wait_ret.kind() == rclcpp::WaitResultKind::Ready) {
        ...
        if (_register_ext_component_reply_sub->take(reply, info)) {
            ...
            request.request_id == reply.request_id)
          {
            ...
            if (reply.success) {
              if (reply.px4_sdk_api_version == kLatestPX4SdkApiVersion) {
                ...
                RCLCPP_FATAL(
                  _node.get_logger(), "Incompatible SDK API version: got %i, expected %i",
                  reply.px4_sdk_api_version, kLatestPX4SdkApiVersion);
              }
