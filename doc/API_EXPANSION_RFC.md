...
Status: Draft for review. This revision reconciles the design against three adversarial review lenses (ROS-native fit, uXRCE-DDS feasibility, aerospace correctness) whose findings are verified against primary source in the appendix and folded into the body below.
...
The SDK is ROS-native first and strictly additive. `px4_ros2_cpp` is treated as the device-driver core, the sole owner of uXRCE-DDS endpoints, exactly as `ros2_control`'s Resource Manager owns hardware and Aerostack2's `AerialPlatform` owns vehicle I/O. Every subscription still flows through the library's `SharedSubscription`; every command still flows through `VehicleCommandSender`. The SDK adds zero new DDS endpoints and zero new DDS traffic. It is one graph with one contract; the ergonomic facade, a BehaviorTree.CPP tree, the CLI, and rqt are all clients of that one graph.
...
### 2.3 The firmware-versus-library skew that the first revision got wrong

This is the single most load-bearing correction in this RFC. The design pinned the reachability wall at `v1.18.0-alpha1` and enumerated the setpoint topics as present, but the SDK's own mandated core requires two topics that this firmware does not bridge.
...
Four strict layers, each additive over the one below. Nothing above L1 opens a DDS endpoint or adds DDS traffic.

**L0 Transport (immovable, not ours).** The uXRCE-DDS bridge, `dds_topics.yaml`, `px4_msgs`, and the Message Translation Node. The hard reachability wall of Section 2.2, corrected per Section 2.3 to require `setpoint_config`/`setpoint_config_reply` for streaming.

**L1 Core (`px4_ros2_cpp`, reused unchanged, additive-only).** The Section 2.1 primitives. The only code that imports `px4_msgs` endpoints or touches DDS. The `ros2_control` ResourceManager / Aerostack2 `AerialPlatform` analogue. Invariant enforced by design: L2 and above cannot import `px4_msgs` endpoints; they consume L1's `TelemetryHub` and `SetpointWriter`/`OffboardSession`, or call the L1 `VehicleCommandSender` and `ModeExecutorBase` primitives directly.
...
- Verdict target: builds, unit-tested, no `/fmu` endpoints beyond L1, all paths verifiable on `v1.18.0-alpha1-20`.
...
Offboard and behaviors are always real operator-selectable registered external modes obtained through the `register_ext_component` handshake, so PX4 remains the resource manager arbitrating exclusive setpoint ownership; two modes can never fight over setpoints. The corrected loss model (Section 4.4) is preserved and stated honestly: on setpoint-stream loss the registered-mode setpoint-update timeout trips and `replaceInternalMode` restores the replaced internal mode; the SDK does not claim the legacy `COM_OF_LOSS_T` offboard-heartbeat behavior it does not implement. Every command `Result` carries the FMU ACK and never claims physical completion; physical completion rides `mode_completed` (asserted `RESULT_FAILURE_OTHER == Result::ModeFailureOther` in `mode.hpp`) for maneuvers, or the latched `~/vehicle_state` for arming. PX4 retains authority over preflight rejection (arming routes through `ModeExecutorBase::arm`/`waitReadyToArm`, not a raw publish). The emergency-stop primitive is explicit, guarded, and clearly labeled irreversible.

---

## 8. Appendix: reconciliation of the adversarial verdicts

Three lenses reviewed the design: ROS-native fit, uXRCE-DDS feasibility, and aerospace correctness. Their headline findings, and how this RFC resolves each, verified against the pinned trees.

### 8.1 Load-bearing corrections applied

1. `setpoint_config`/`setpoint_config_reply` firmware skew (feasibility + aerospace, both lenses independently). Verified: absent from `v1.18.0-alpha1-20` `dds_topics.yaml` (grep clean) and no `SetpointConfig.msg` in that tree, yet required by `px4_ros2_cpp` 1.17.0 (`message_compatibility_check.hpp:32,48`) and gating `setActive(true)` (`mode.cpp:60-64`, comment `mode.cpp:407`). Resolution: L0 inventory corrected to require these two topics for streaming; streaming phases pinned to the firmware line that bridges them; the first PR scoped to non-streaming surfaces verifiable now (Sections 2.3, 6). The design's premise is no longer internally inconsistent.
2. Offboard failsafe model (aerospace). Verified: zero hits for `offboard_control_mode`/`COM_OF_LOSS`/`NAVIGATION_STATE_OFFBOARD` in `px4_ros2_cpp`. Registered external modes are not governed by the legacy offboard heartbeat. Resolution: thesis and Capability 6 reframed to registered-mode setpoint-update-timeout + `replaceInternalMode`; the false ">2 Hz `OffboardControlMode` heartbeat / `COM_OF_LOSS_T` fallback" mental model is removed (Section 4.4).
3. External estimate injection (aerospace). Verified: only `local_position_measurement_interface` (-> `vehicle_visual_odometry`) and `global_position_measurement_interface` (-> `aux_global_position`) exist in L1. Resolution: capability restricted to those two; mocap/flow/distance/obstacle moved to RFC future work requiring an upstream L1 extension, with EKF2 frame/covariance correctness as acceptance criteria (Sections 5.3, 5.1).
4. VTOL through `VehicleCommandSender` versus "L1 unchanged" (aerospace). Verified: `vtol.hpp:86` uses an ad-hoc `_vehicle_command_pub` with bool returns and no ACK. Resolution: L2 calls `VehicleCommandSender` directly rather than modifying the L1 `vtol.hpp` helper, achieving uniform ACK/Result while preserving "L1 reused unchanged" (Section 5.1). Gimbal-manager Timeout behavior documented.
5. Orbit reframe (all three lenses). Verified: DO_ORBIT=34 is a one-shot `vehicle_command` with no `mode_completed` and no orbit-status topic. Resolution: the action is given an explicit terminal condition (revolutions or duration), `angle_deg` feedback restated as SDK-computed from `vehicle_local_position`, and cancel implemented as an explicit mode switch (Section 5.1).
6. GoTo reframe (feasibility + aerospace). Resolution: the two mechanisms (streamed `goto_setpoint`, multicopter-only, `setpoint_config`-dependent, SDK-computed completion; versus one-shot DO_REPOSITION=192 for global/fixed-wing/rover) are documented as distinct with the frame/airframe split surfaced on `~/capabilities` (Section 5.1).

### 8.2 Additions folded in from the gap notes

tf2 broadcaster; a typed node-parameter library distinct from FMU parameters; time-offset via `/diagnostics` instead of a bespoke topic; geofence-breach read via `failsafe_flags`; explicit emergency-stop, set-mode-to-safe, and set-home services; and the reachable-but-omitted telemetry (`transponder_report`, `collision_constraints`, `wind`, `airspeed_validated`, `manual_control_setpoint`) plus fixed-wing and rover offboard setpoint subtypes (Sections 5.1, 5.4). The `arming_check_request` event-on-request semantics are documented so `/diagnostics` treats absence as "unknown".

### 8.3 Surfaces confirmed sound and kept unchanged

Bringup and message-compat gating; Arm/Disarm; Takeoff/Land/RTL (the strongest surface, PX4-owned maneuvers on `mode_completed`); vehicle-state and standard-types telemetry with single-place frame conversion; health/failsafe monitoring honest about the missing event topic; time-base reconciliation; custom behaviors via pluginlib with a documented external-mode slot ceiling; and the BT-plus-simple-commander orchestration surface with one contract and no parallel path. All reachability claims for these were verified true against the pinned `dds_topics.yaml` (66 entries: 28 fmu/out including the commented `vehicle_angular_velocity`, 38 fmu/in, no services block), and the non-goals were confirmed absent by grep rather than assumed.

### 8.4 Independent upstream note

`SharedSubscription::Key::operator<` (`shared_subscription.hpp:78`) is not a strict weak ordering; recommended as an independent L1 fix since the whole SDK sits on that map (Section 5.5).
