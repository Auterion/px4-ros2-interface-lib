  ...
  void updateSetpoint(float dt_s) override
  {
    ...
    switch (_state) {
      ...
      case State::GoingNorth: {
        ...
        if (px4_ros2::horizontalDistanceToGlobalPosition(_vehicle_global_position->position(),
 ...
 private:
  ...
  bool positionReached(const Eigen::Vector3d& target_position_m) const
  {
    ...
    const float position_error =
        px4_ros2::distanceToGlobalPosition(_vehicle_global_position->position(), target_position_m);
  ...
  bool headingReached(float target_heading_rad) const
  {
    ...
    const float heading_error_wrapped =
        px4_ros2::wrapPi(target_heading_rad - _vehicle_attitude->yaw());
