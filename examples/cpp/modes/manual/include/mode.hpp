...
class FlightModeTest : public px4_ros2::ModeBase {
 public:
  ...
  void updateSetpoint(float dt_s) override
  {
    ...
    const bool want_rates = fabsf(_manual_control_input->roll()) > threshold ||
                            fabsf(_manual_control_input->pitch()) > threshold;
    ...
    if (want_rates) {
      ...
      const Eigen::Vector3f rates_sp{px4_ros2::degToRad(_manual_control_input->roll() * 500.f),
      ...
      const Eigen::Quaternionf qd = px4_ros2::eulerRpyToQuaternion(
          ...
          px4_ros2::degToRad(-_manual_control_input->pitch() * 55.f), _yaw);
