...
class FlightModeTest : public px4_ros2::ModeBase {
 public:
  explicit FlightModeTest(rclcpp::Node& node) : ModeBase(node, Settings{kName}.preventArming(true))
  {
  ...
  void updateSetpoint(float dt_s) override
  {
    ...
    switch (_state) {
      ...
      case State::GoingNorth: {
        ...
        const Eigen::Vector2f vehicle_to_target_xy =
            target_position_m.head(2) - _vehicle_local_position->positionNed().head(2);
      ...
      case State::GoingEast: {
        ...
        _goto_setpoint->update(target_position_m, _spinning_heading_rad,
                               max_horizontal_velocity_m_s, max_vertical_velocity_m_s,
      ...
      case State::GoingSouthwest: {
        ...
        const Eigen::Vector2f position_of_interest_m =
            _start_position_m.head(2) + Eigen::Vector2f{kTriangleHeight, 0.f};
        const Eigen::Vector2f vehicle_to_poi_xy =
            position_of_interest_m - _vehicle_local_position->positionNed().head(2);
 ...
 private:
  ...
  enum class State { SettlingAtStart = 0, GoingNorth, GoingEast, GoingSouthwest } _state;
