...
static const std::string kNodeName = "rover_cruise_control_node";

#ifndef M_PI_F
#define M_PI_F static_cast<float>(M_PI)
#endif

class RoverCruiseControlMode : public px4_ros2::ModeBase
{
public:
  explicit RoverCruiseControlMode(rclcpp::Node & node)
  : ModeBase(node, kName)
  {
    _manual_control_input = std::make_shared<px4_ros2::ManualControlInput>(*this);
    _cruise_control = std::make_shared<px4_ros2::CruiseControlSetpointType>(*this);
  }

  void onActivate() override
  {
    _state = _cruise_control->initialize_state();
  ...
  void updateSetpoint(float dt_s) override
  {
    ...
    _angular_velocity_z = _manual_control_input->roll() * _max_angular_velocity_z;

    // Update cruise control with the new velocities
    _cruise_control->update(_linear_velocity_x, _angular_velocity_z);
  }
...
private:
  ...
  std::shared_ptr<px4_ros2::CruiseControlSetpointType> _cruise_control;
...
class rover_cruise_control_node : public rclcpp::Node
{
public:
  rover_cruise_control_node()
  : Node(kNodeName)
  {
    // Enable debug output
    auto ret =
    ...
    _mode = std::make_unique<RoverCruiseControlMode>(*this);
...
private:
  std::unique_ptr<RoverCruiseControlMode> _mode;
};
