...
class GlobalNavigationTest : public px4_ros2::GlobalPositionMeasurementInterface {
 public:
  explicit GlobalNavigationTest(rclcpp::Node& node) : GlobalPositionMeasurementInterface(node)
  {
    _timer = node.create_wall_timer(1s, [this] { updateGlobalPosition(); });
  ...
  void updateGlobalPosition()
  {
    ...
    try {
      ...
      RCLCPP_ERROR_THROTTLE(_node.get_logger(), *_node.get_clock(), 1000, "Exception caught: %s",
                            e.what());
    }
  }
...
class ExampleGlobalNavigationNode : public rclcpp::Node {
 public:
  ExampleGlobalNavigationNode() : Node("example_global_navigation_node")
  {
    // Enable debug output
    auto ret =
