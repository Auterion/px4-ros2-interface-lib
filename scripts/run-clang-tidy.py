...
def main():
    ...
    parser.add_argument(
        "-clang-apply-replacements-binary", metavar="PATH", help="path to clang-apply-replacements binary"
    )
    ...
    parser.add_argument("-line-filter", default=None, help="List of files with line ranges to filter the" "warnings.")
    if yaml:
        parser.add_argument(
            "-export-fixes",
            metavar="filename",
    ...
    parser.add_argument("-ignore", default="", help="File path to clang-tidy-ignore")
    ...
    try:
        ...
        for _ in range(max_task):
            t = threading.Thread(
                target=run_tidy, args=(args, clang_tidy_binary, tmpdir, build_path, task_queue, lock, failed_files)
            )
            t.daemon = True
            t.start()
